...
//$doman = explode('@', $request_data['email'],2)[1];
$text = explode('@', $request_data['email'], 2)[1];
debug_to_console($text);
	if (!preg_match('/^[a-zA-Z0-9_.+-]+@(?:(?:[a-zA-Z0-9-]+\.)?[a-zA-Z]+\.)?(capgemini)\.com$/', $request_data['email'] )){
		return __( 'You must enter a Capgemini email adress, "@capgemini.com"', 'profile-builder');
	}
